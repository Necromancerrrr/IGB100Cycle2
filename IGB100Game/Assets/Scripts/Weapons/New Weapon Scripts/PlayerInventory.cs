...
public class PlayerInventory : MonoBehaviour
{
    ...
    public Item Get(ItemData type)
    {
        ...
        if(type is PassiveData) return Get(type as PassiveData);
        return null;
    }
    ...
    public int Add(WeaponData data)
    {
        ...
        if (weaponType != null) 
        {
            ...
            Weapon spawnedWepaon = (Weapon)go.AddComponent(weaponType);
            spawnedWepaon.Initialize(data);
            spawnedWepaon.transform.SetParent(transform);
            spawnedWepaon.transform.localPosition = Vector2.zero;
            spawnedWepaon.OnEquip();

            weaponSlots[slotNum].Assign(spawnedWepaon);

            if(GameManager.instance != null && GameManager.instance.isChoosingLvlUp) GameManager.instance.EndLevelUp();
    ...
    public int Add(PassiveData data)
    {
        ...
        if(GameManager.instance != null && GameManager.instance.isChoosingLvlUp)
        {
